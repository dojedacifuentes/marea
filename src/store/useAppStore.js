...
const useAppStore = create((set, get) => ({
  ...
  addInterrogacion: (int) => {
    ...
    set({ interrogaciones: next })
    storage.set('interrogaciones', next)
  },
  ...
  estudiarFlashcard: (id, correcto) => {
    ...
    if (!card) return
    const updates = {
      vecesEstudiada: card.vecesEstudiada + 1,
      correctas: correcto ? card.correctas + 1 : card.correctas,
      estado: correcto ? 'sabida' : 'repasar',
    }
    get().updateFlashcard(id, updates)
  ...
  _checkRecompensa: (tipo) => {
    ...
    if (tipo === 'tarea_completada') {
      puntos += 10
      tareasCompletadas += 1
      if (tareasCompletadas % 5 === 0) {
        const newSticker = allStickers.find((s) => !stickers.includes(s))
        if (newSticker) stickers.push(newSticker)
      }
    }
    if (tipo === 'flashcard') {
      puntos += 5
      flashcardsEstudiadas += 1
      if (flashcardsEstudiadas % 10 === 0) {
        const newSticker = allStickers.find((s) => !stickers.includes(s))
        if (newSticker) stickers.push(newSticker)
      }
    }
    ...
    set({ recompensas: next })
    storage.set('recompensas', next)
  },
  ...
  addFocusSession: (session) => {
    ...
    set({ focusSessions: next })
    storage.set('focusSessions', next)
    get()._checkRecompensa('focus')
  },
  ...
  resetToDemo: () => {
    ...
    set({
      alumnos: demoAlumnos,
      interrogaciones: demoInterrogaciones,
      tareas: demoTareas,
      lecturas: demoLecturas,
      notas: demoNotas,
      biblioteca: demoBiblioteca,
      flashcards: demoFlashcards,
      timeline: demoTimeline,
      autores: demoAutores,
      recompensas: demoRecompensas,
    })
  },
}))
